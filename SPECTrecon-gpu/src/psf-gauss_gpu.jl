...
export psf_gauss_gpu

"""
    psf_gauss_gpu( ; ny, px, pz, fwhm_start, fwhm_end, fwhm, fwhm_x, fwhm_z, T)
...
function psf_gauss_gpu( ;
    ...
    psf = CuArray(zeros(T, px, pz, ny))

    rx = (-(px-1)÷2):((px-1)÷2)
    rz = (-(pz-1)÷2):((pz-1)÷2)
    for iy in 1:ny # depth-dependent blur
        psf[:,:,iy] = psf_gauss_gpu(rx, fwhm_x[iy]) * psf_gauss_gpu(rz, fwhm_z[iy])'
    end
    return psf
end

function psf_gauss_gpu(r::AbstractVector, fwhm::Real)
   if fwhm == 0
